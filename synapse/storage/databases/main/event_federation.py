...
class EventFederationWorkerStore(SignatureWorkerStore, EventsWorkerStore, SQLBaseStore):
    ...
    def _get_auth_chain_difference_using_cover_index_txn(
        ...
        # start measure 1 here
        with Measure(self.hs.get_clock(), "_get_auth_chain_difference_using_cover_index.section_1"):
        ...
        # start measure 2 here
        with Measure(self.hs.get_clock(), "_get_auth_chain_difference_using_cover_index.section_2"):
            # Now we look up all links for the chains we have, adding chains that
            # are reachable from any event.
        ...
        # start measure 3 here
        with Measure(
            self.hs.get_clock(),
            "_get_auth_chain_difference_using_cover_index.section_3"
            ):
            if isinstance(self.database_engine, PostgresEngine):
                # We can use `execute_values` to efficiently fetch the gaps when
                # using postgres.
